...
instance Print TypeIdentAbstract where
  prt _ (TypeIdentAbstract i) = doc (showString i)
...
instance Print (TopDef a) where
  prt i e = case e of
    FnDef _ typename id args block -> prPrec i 0 (concatD [prt 0 typename, prt 0 id, doc (showString "("), prt 0 args, doc (showString ")"), prt 0 block])
  prtList _ [x] = concatD [prt 0 x]
...
instance Print (Arg a) where
  prt i e = case e of
    Arg _ typename id -> prPrec i 0 (concatD [prt 0 typename, prt 0 id])
  prtList _ [] = concatD []
  prtList _ [x] = concatD [prt 0 x]
  prtList _ (x:xs) = concatD [prt 0 x, doc (showString ","), prt 0 xs]
...
instance Print (Stmt a) where
  prt i e = case e of
    ...
    Decl _ typename items -> prPrec i 0 (concatD [prt 0 typename, prt 0 items, doc (showString ";")])
...
instance Print (TypName a) where
  prt i e = case e of
    Fun _ typename typenames -> prPrec i 0 (concatD [prt 0 typename, doc (showString "("), prt 0 typenames, doc (showString ")")])
...
instance Print (TypeParam a) where
  prt i e = case e of
    TypeParamJust _ typeidentabstracts -> prPrec i 0 (concatD [doc (showString "("), prt 0 typeidentabstracts, doc (showString ")")])
    TypeParamJustOne _ typeidentabstract -> prPrec i 0 (concatD [prt 0 typeidentabstract])
    TypeParamNone _ -> prPrec i 0 (concatD [])

instance Print [TypeIdentAbstract] where
  prt = prtList

instance Print (TypeArgEl a) where
  prt i e = case e of
    TypeArgEl _ typeexpression -> prPrec i 0 (concatD [prt 0 typeexpression])
  prtList _ [x] = concatD [prt 0 x]
  prtList _ (x:xs) = concatD [prt 0 x, doc (showString ","), prt 0 xs]
...
instance Print (TypeArg a) where
  prt i e = case e of
    TypeArgJust _ typeargel typeargels -> prPrec i 0 (concatD [doc (showString "("), prt 0 typeargel, doc (showString ","), prt 0 typeargels, doc (showString ")")])
    TypeArgJustOne _ typesimpleexpression -> prPrec i 0 (concatD [prt 0 typesimpleexpression])

instance Print (TypeSimpleExpression a) where
  prt i e = case e of
    TypeSExprEmpty _ -> prPrec i 0 (concatD [doc (showString "void")])
    TypeSExprAbstract _ typeidentabstract -> prPrec i 0 (concatD [prt 0 typeidentabstract])
    TypeSExprIdent _ id -> prPrec i 0 (concatD [prt 0 id])
    TypeSExprList _ typeexpression -> prPrec i 0 (concatD [doc (showString "["), prt 0 typeexpression, doc (showString "]")])

instance Print (TypeExpression a) where
  prt i e = case e of
    TypeExprSimple _ typesimpleexpression -> prPrec i 4 (concatD [prt 0 typesimpleexpression])
    TypeExprIdent _ typearg id -> prPrec i 3 (concatD [prt 0 typearg, prt 0 id])
    TypeExprTuple _ typeexpression typeexpressions -> prPrec i 1 (concatD [prt 2 typeexpression, doc (showString "*"), prt 2 typeexpressions])
    TypeFun _ typeexpression1 typeexpression2 -> prPrec i 0 (concatD [prt 1 typeexpression1, doc (showString "->"), prt 0 typeexpression2])
  prtList 2 [x] = concatD [prt 2 x]
  prtList 2 (x:xs) = concatD [prt 2 x, doc (showString "*"), prt 2 xs]

instance Print (TypeConstraint a) where
  prt i e = case e of
    TypeConstrDef _ typeexpression -> prPrec i 0 (concatD [doc (showString ":"), prt 0 typeexpression])
    TypeConstrEmpty _ -> prPrec i 0 (concatD [])

instance Print (ASTNode a) where
  prt i e = case e of
    ASTProgram _ program -> prPrec i 0 (concatD [prt 0 program])
    ASTExpr _ expr -> prPrec i 0 (concatD [prt 0 expr])
    ASTStmt _ stmt -> prPrec i 0 (concatD [prt 0 stmt])
    ASTBlock _ block -> prPrec i 0 (concatD [prt 0 block])
    ASTTopDef _ topdef -> prPrec i 0 (concatD [prt 0 topdef])
    ASTTypeName _ typename -> prPrec i 0 (concatD [prt 0 typename])
    ASTItem _ item -> prPrec i 0 (concatD [prt 0 item])
    ASTNone _ -> prPrec i 0 (concatD [doc (showString "empty-ast")])
